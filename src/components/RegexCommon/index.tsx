    ...
    <div>
      {/* {regexData.map((item, index) => (
        <div className='mb-4' key={index}>
          ...
          <p className='text-[1.4rem] text-white'>{item.value}</p>
          {item.child?.length > 0 ? (
            <ul>
              {item.child.map((it) => (
                <li key={it.name} className='mb-4 pl-8'>
                  ...
                  <pre>
                    <code>
                      <span className='pl-4 text-[1.4rem] text-white'>{it.data}</span>
                    </code>
                  </pre>
                </li>
              ))}
      ...
      <h1 className='text-[3rem] text-second'>Coming soon ...</h1>
    </div>
  );
}
